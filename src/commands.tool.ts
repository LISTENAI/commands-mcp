...
import { $ as $kleur, blue, bold, green, red, white } from 'kleur/colors';
...
export function createCommandTool(name: string, spec: Command) {
  ...
  class DynamicCommandsTool {
    ...
    async execute(args: Args, _context: Context, _req: Request): Promise<CallToolResult> {
      ...
      console.error(bold(white(`$ ${command}`)));
      ...
      const terminateByTimeout = ((timeout) => timeout ? setTimeout(() => {
        console.error(bold(blue(`> Command terminated after timeout: ${timeout}ms`)));
      ...
      const terminateByOutput = ((pattern) => {
        if (!pattern) {
          ...
          return (line: string) => {
            if (regex.test(line)) {
              console.error(bold(blue(`> Command terminated due to output matching: ${pattern}`)));
              proc.kill(spec.terminate?.signal);
            }
          };
        } else {
          return (line: string) => {
            if (line.includes(pattern)) {
              console.error(bold(blue(`> Command terminated due to output containing: ${pattern}`)));
              proc.kill(spec.terminate?.signal);
            }
          };
        }
      ...
      if (code == 0) {
        console.error(bold(green('> Command executed successfully')));
      } else {
