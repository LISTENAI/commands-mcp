...
impl Read for Device {
    fn read(&mut self, buf: &mut [u8]) -> IoResult<usize> {
        self.handle
            .read_bulk(self.interface.ep_in.address, buf, Duration::from_secs(1))
    ...
    fn read_to_end(&mut self, buf: &mut Vec<u8>) -> IoResult<usize> {
        let mut total = 0;
        loop {
            let mut chunk = vec![0; self.interface.ep_in.mps as usize];
            let len = self.read(&mut chunk)?;
            if len < self.interface.ep_in.mps as usize {
                break;
            }
...
impl Write for Device {
    fn write(&mut self, buf: &[u8]) -> IoResult<usize> {
        eprintln!(
            "Requested to write {} bytes to device {}",
            buf.len(),
            self.id
        );
        ...
        if self.write_buf.len() >= write_len {
            ...
            eprintln!(
                "Actual wrote {} bytes, remaining {} bytes",
                write_len,
                self.write_buf.len()
            );
        }
